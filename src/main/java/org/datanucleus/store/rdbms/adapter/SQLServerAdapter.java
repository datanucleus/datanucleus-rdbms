...
/**
 * Provides methods for adapting SQL language elements to the Microsoft SQL Server database. Note that
 * majorVersion from JDBC doesn't align to version number of SQLServer. 1995 = "6", 1998 = "7", 2000/2003 =
 * "8", 2005 = "9", 2008 = "10", 2012 = "11", 2014 = "12", ...
 * @see BaseDatastoreAdapter
 */
public class SQLServerAdapter extends BaseDatastoreAdapter
{
    /**
     * Microsoft SQL Server 2000 uses reserved keywords for defining, manipulating, and accessing databases.
     * Reserved keywords are part of the grammar of the Transact-SQL language used by SQL Server to parse and
     * understand Transact-SQL statements and batches. Although it is syntactically possible to use SQL Server
     * reserved keywords as identifiers and object names in Transact-SQL scripts, this can be done only using
    ...
    /**
     * Returns the appropriate DDL to create an index. Overrides the superclass variant since Postgresql
     * doesn't support having index names specified in a particular schema (i.e "{schema}.{indexName}").
    ...
    {
        /**
         * CREATE [UNIQUE] [CLUSTERED | NONCLUSTERED] INDEX index_name ON tableName (column [ASC|DESC] [ ,...n
         * ]) [ INCLUDE (column [ ,...n ] )] [ WHERE <filter_predicate> ] [ WITH ( <relational_index_option> [
         * ,...n ] ) ] [ ON { partition_scheme_name (column_name) | filegroup_name | default }]
         */
     ...
     * This is the case when we have a single column in the table and that column is autoincrement/identity
     * (and so is assigned automatically in the datastore).
     ...
     * ranges using the OFFSET/FETCH keywords.
    ...
    public String getRangeByLimitEndOfStatementClause(long offset, long count, boolean hasOrdering)
    {
        ...
        {
            // SQLServer requires ORDER BY to be able to use OFFSET
    ...
    public String getSchemaName(Connection conn) throws SQLException
    {
        ...
        /*
         * As of version 7 there was no equivalent to the concept of "schema" in SQL Server. For
         * DatabaseMetaData functions that include SCHEMA_NAME drivers usually return the user name that owns
         * the table. So the default ProbeTable method for determining the current schema just ends up
         * returning the current user. If we then use that name in performing metadata queries our results may
         * get filtered down to just objects owned by that user. So instead we report the schema name as null
         * which should cause those queries to return everything. DO not use the user name here, as in MSSQL,
         * you are able to use an user name and access any schema. Use an empty string, otherwise
         * fullyqualified object name is invalid In MSSQL, fully qualified must include the object owner, or
         * an empty owner <catalog>.<schema>.<object> e.g. mycatalog..mytable
         */
        return "";
    }
    ...
    /*
     * (non-Javadoc)
     * @see org.datanucleus.store.rdbms.adapter.BaseDatastoreAdapter#getSQLMethodClass(java.lang.String,
     * java.lang.String)
     */
    @Override
    public Class getSQLMethodClass(String className, String methodName, ClassLoaderResolver clr)
    {
        if (className == null)
        {
            ...
            {
                return org.datanucleus.store.rdbms.sql.method.SQLCubeFunction.class;
            }
            else if ("SQL_rollup".equals(methodName))
            {
                return org.datanucleus.store.rdbms.sql.method.SQLRollupFunction.class;
            }
            else if ("DAY_OF_WEEK".equals(methodName))
            {
                return org.datanucleus.store.rdbms.sql.method.TemporalDayOfWeekMethod4.class;
            }
            else if ("HOUR".equals(methodName))
            {
                return org.datanucleus.store.rdbms.sql.method.TemporalHourMethod4.class;
            }
            else if ("MINUTE".equals(methodName))
            {
                return org.datanucleus.store.rdbms.sql.method.TemporalMinuteMethod4.class;
            }
            else if ("SECOND".equals(methodName))
            {
                return org.datanucleus.store.rdbms.sql.method.TemporalSecondMethod4.class;
            }
        }
        else
        {
            ...
            if ("java.lang.String".equals(className))
            {
                ...
                {
                    return org.datanucleus.store.rdbms.sql.method.StringConcat2Method.class;
                }
                else if ("indexOf".equals(methodName))
                {
                    return org.datanucleus.store.rdbms.sql.method.StringIndexOf4Method.class;
                }
                else if ("length".equals(methodName))
                {
                    return org.datanucleus.store.rdbms.sql.method.StringLength4Method.class;
                }
                else if ("startsWith".equals(methodName))
                {
                    return org.datanucleus.store.rdbms.sql.method.StringStartsWith2Method.class;
                }
                else if ("substring".equals(methodName))
                {
                    return org.datanucleus.store.rdbms.sql.method.StringSubstring4Method.class;
                }
                else if ("trim".equals(methodName))
                {
                    return org.datanucleus.store.rdbms.sql.method.StringTrim2Method.class;
                }
            }
            else if ("java.util.Date".equals(className) || (cls != null && java.util.Date.class.isAssignableFrom(cls)))
            {
                ...
                else if ("getHour".equals(methodName))
                {
                    return org.datanucleus.store.rdbms.sql.method.TemporalHourMethod4.class;
                }
                else if ("getMinute".equals(methodName))
                {
                    return org.datanucleus.store.rdbms.sql.method.TemporalMinuteMethod4.class;
                }
                else if ("getSecond".equals(methodName))
                {
                    return org.datanucleus.store.rdbms.sql.method.TemporalSecondMethod4.class;
                }
            }
            ...
            else if ("java.time.LocalDate".equals(className) && "getDayOfWeek".equals(methodName))
            {
                return org.datanucleus.store.rdbms.sql.method.TemporalDayOfWeekMethod4.class;
            }
    ...
    public Class getSQLOperationClass(String operationName)
    {
        ...
        {
            return org.datanucleus.store.rdbms.sql.operation.Concat2Operation.class;
        }
    ...
    public void initialiseTypes(StoreSchemaHandler handler, ManagedConnection mconn)
    {
        ...
        SQLTypeInfo sqlType = new org.datanucleus.store.rdbms.adapter.SQLServerTypeInfo(
                "UNIQUEIDENTIFIER", (short) Types.CHAR, 36, "'", "'", "", 1, false, (short) 2, false, false, false, "UNIQUEIDENTIFIER", (short) 0, (short) 0,
                10);
        ...
        sqlType = new org.datanucleus.store.rdbms.adapter.SQLServerTypeInfo(
                "IMAGE", (short) Types.LONGVARBINARY, 2147483647, null, null, null, 1, false, (short) 1, false, false, false, "LONGVARBINARY", (short) 0,
                (short) 0, 0);
    ...
    protected void loadColumnMappings(PluginManager mgr, ClassLoaderResolver clr)
    {
        ...
        registerColumnMapping(Boolean.class.getName(), org.datanucleus.store.rdbms.mapping.column.BooleanColumnMapping.class, JDBCType.BOOLEAN, "BOOLEAN",
            false);
        registerColumnMapping(Boolean.class.getName(), org.datanucleus.store.rdbms.mapping.column.SmallIntColumnMapping.class, JDBCType.SMALLINT, "SMALLINT",
            false);

        registerColumnMapping(Byte.class.getName(), org.datanucleus.store.rdbms.mapping.column.SmallIntColumnMapping.class, JDBCType.SMALLINT, "SMALLINT",
            false);
        ...
        registerColumnMapping(Double.class.getName(), org.datanucleus.store.rdbms.mapping.column.DecimalColumnMapping.class, JDBCType.DECIMAL, "DECIMAL",
            false);
        ...
        registerColumnMapping(Integer.class.getName(), org.datanucleus.store.rdbms.mapping.column.NumericColumnMapping.class, JDBCType.NUMERIC, "NUMERIC",
            false);
        registerColumnMapping(Integer.class.getName(), org.datanucleus.store.rdbms.mapping.column.SmallIntColumnMapping.class, JDBCType.SMALLINT, "SMALLINT",
            false);
        ...
        registerColumnMapping(Long.class.getName(), org.datanucleus.store.rdbms.mapping.column.SmallIntColumnMapping.class, JDBCType.SMALLINT, "SMALLINT",
            false);

        registerColumnMapping(Short.class.getName(), org.datanucleus.store.rdbms.mapping.column.SmallIntColumnMapping.class, JDBCType.SMALLINT, "SMALLINT",
            true);
        ...
        registerColumnMapping(String.class.getName(), org.datanucleus.store.rdbms.mapping.column.LongVarcharColumnMapping.class, JDBCType.LONGVARCHAR,
            "LONGVARCHAR", false);
        ...
        registerColumnMapping(String.class.getName(), org.datanucleus.store.rdbms.mapping.column.CharColumnMapping.class, JDBCType.CHAR, "UNIQUEIDENTIFIER",
            false);
        registerColumnMapping(String.class.getName(), org.datanucleus.store.rdbms.mapping.column.NVarcharColumnMapping.class, JDBCType.NVARCHAR, "NVARCHAR",
            false);
        ...
        registerColumnMapping(BigDecimal.class.getName(), org.datanucleus.store.rdbms.mapping.column.DecimalColumnMapping.class, JDBCType.DECIMAL, "DECIMAL",
            true);
        registerColumnMapping(BigDecimal.class.getName(), org.datanucleus.store.rdbms.mapping.column.NumericColumnMapping.class, JDBCType.NUMERIC, "NUMERIC",
            false);

        registerColumnMapping(BigInteger.class.getName(), org.datanucleus.store.rdbms.mapping.column.NumericColumnMapping.class, JDBCType.NUMERIC, "NUMERIC",
            true);
        ...
        registerColumnMapping(java.sql.Date.class.getName(), org.datanucleus.store.rdbms.mapping.column.TimestampColumnMapping.class, JDBCType.TIMESTAMP,
            "TIMESTAMP", false);
        ...
        registerColumnMapping(java.sql.Date.class.getName(), org.datanucleus.store.rdbms.mapping.column.VarCharColumnMapping.class, JDBCType.VARCHAR, "VARCHAR",
            false);

        registerColumnMapping(java.sql.Date.class.getName(), org.datanucleus.store.rdbms.mapping.column.BigIntColumnMapping.class, JDBCType.BIGINT, "BIGINT",
            false);
        ...
        registerColumnMapping(java.sql.Time.class.getName(), org.datanucleus.store.rdbms.mapping.column.TimestampColumnMapping.class, JDBCType.TIMESTAMP,
            "TIMESTAMP", false);
        ...
        registerColumnMapping(java.sql.Time.class.getName(), org.datanucleus.store.rdbms.mapping.column.VarCharColumnMapping.class, JDBCType.VARCHAR, "VARCHAR",
            false);
        registerColumnMapping(java.sql.Time.class.getName(), org.datanucleus.store.rdbms.mapping.column.BigIntColumnMapping.class, JDBCType.BIGINT, "BIGINT",
            false);

        registerColumnMapping(java.sql.Timestamp.class.getName(), org.datanucleus.store.rdbms.mapping.column.TimestampColumnMapping.class, JDBCType.TIMESTAMP,
            "TIMESTAMP", true);
        registerColumnMapping(java.sql.Timestamp.class.getName(), org.datanucleus.store.rdbms.mapping.column.CharColumnMapping.class, JDBCType.CHAR, "CHAR",
            false);
        registerColumnMapping(java.sql.Timestamp.class.getName(), org.datanucleus.store.rdbms.mapping.column.VarCharColumnMapping.class, JDBCType.VARCHAR,
            "VARCHAR", false);
        registerColumnMapping(java.sql.Timestamp.class.getName(), org.datanucleus.store.rdbms.mapping.column.DateColumnMapping.class, JDBCType.DATE, "DATE",
            false);
        registerColumnMapping(java.sql.Timestamp.class.getName(), org.datanucleus.store.rdbms.mapping.column.TimeColumnMapping.class, JDBCType.TIME, "TIME",
            false);

        registerColumnMapping(java.util.Date.class.getName(), org.datanucleus.store.rdbms.mapping.column.TimestampColumnMapping.class, JDBCType.TIMESTAMP,
            "TIMESTAMP", true);
        ...
        registerColumnMapping(java.util.Date.class.getName(), org.datanucleus.store.rdbms.mapping.column.VarCharColumnMapping.class, JDBCType.VARCHAR,
            "VARCHAR", false);
        ...
        registerColumnMapping(java.util.Date.class.getName(), org.datanucleus.store.rdbms.mapping.column.BigIntColumnMapping.class, JDBCType.BIGINT, "BIGINT",
            false);

        registerColumnMapping(java.io.Serializable.class.getName(), org.datanucleus.store.rdbms.mapping.column.VarBinaryColumnMapping.class, JDBCType.VARBINARY,
            "VARBINARY", true);
        registerColumnMapping(java.io.Serializable.class.getName(), org.datanucleus.store.rdbms.mapping.column.BlobColumnMapping.class, JDBCType.BLOB, "BLOB",
            false);
        registerColumnMapping(java.io.Serializable.class.getName(), org.datanucleus.store.rdbms.mapping.column.VarBinaryColumnMapping.class, JDBCType.VARBINARY,
            "VARBINARY", false);
        registerColumnMapping(java.io.Serializable.class.getName(), org.datanucleus.store.rdbms.mapping.column.LongVarBinaryColumnMapping.class,
            JDBCType.LONGVARBINARY, "LONGVARBINARY", false);

        registerColumnMapping(byte[].class.getName(), org.datanucleus.store.rdbms.mapping.column.VarBinaryColumnMapping.class, JDBCType.VARBINARY, "VARBINARY",
            true);
        ...
        registerColumnMapping(byte[].class.getName(), org.datanucleus.store.rdbms.mapping.column.VarBinaryColumnMapping.class, JDBCType.VARBINARY, "VARBINARY",
            false);
        registerColumnMapping(byte[].class.getName(), org.datanucleus.store.rdbms.mapping.column.LongVarBinaryColumnMapping.class, JDBCType.LONGVARBINARY,
            "LONGVARBINARY", false);

        registerColumnMapping(java.io.File.class.getName(), org.datanucleus.store.rdbms.mapping.column.BinaryStreamColumnMapping.class, JDBCType.LONGVARBINARY,
            "LONGVARBINARY", true);

        registerColumnMapping(DatastoreId.class.getName(), org.datanucleus.store.rdbms.mapping.column.BigIntColumnMapping.class, JDBCType.BIGINT, "BIGINT",
            true);
        registerColumnMapping(DatastoreId.class.getName(), org.datanucleus.store.rdbms.mapping.column.NumericColumnMapping.class, JDBCType.NUMERIC, "NUMERIC",
            false);
        ...
        registerColumnMapping(DatastoreId.class.getName(), org.datanucleus.store.rdbms.mapping.column.VarCharColumnMapping.class, JDBCType.VARCHAR, "VARCHAR",
            false);
        registerColumnMapping(DatastoreId.class.getName(), org.datanucleus.store.rdbms.mapping.column.IntegerColumnMapping.class, JDBCType.INTEGER, "INT",
            true);
    ...
    /**
     * Method to create a column info for the current row. Overrides the dataType/columnSize/decimalDigits to
     * cater for SQLServer particularities.
    ...
    /**
     * Method to return if it is valid to select the specified mapping for the specified statement for this
     * datastore adapter. Sometimes, dependent on the type of the column(s), and what other components are
     * present in the statement, it may be invalid to select the mapping. This implementation returns true, so
     * override in database-specific subclass as required.
