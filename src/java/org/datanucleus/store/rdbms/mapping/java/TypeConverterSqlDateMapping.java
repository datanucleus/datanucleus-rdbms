...
/**
 * Mapping where the member has its value converted to/from some storable datastore type using a TypeConverter using java.sql.Date in the datastore.
 */
public class TypeConverterSqlDateMapping extends TypeConverterMapping
{
    /* (non-Javadoc)
    ...
    public void initialize(RDBMSStoreManager storeMgr, String type)
    {
        ...
        if (!Date.class.isAssignableFrom(datastoreType))
        {
            throw new NucleusException("Attempt to create mapping for type " + type + " yet this is not using Date in the datastore");
        }
    }
    ...
    public void initialize(AbstractMemberMetaData mmd, Table table, ClassLoaderResolver clr, TypeConverter conv)
    {
        ...
        if (!Date.class.isAssignableFrom(datastoreType))
        {
            throw new NucleusException("Attempt to create mapping for member " + mmd.getFullFieldName() + " yet this is not using Date in the datastore");
        }
    }
